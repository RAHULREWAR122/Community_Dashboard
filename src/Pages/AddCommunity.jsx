...
function AddCommunity({ setShowForm, showForm }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const req = await axios.post(`${api}/addCommunity`, data, {
        headers: {
          Authorization: `${token}`,
        },
      });
      ...
      if (req.status === 201) {
        alert("community create successfully");
        setShowForm(false);
      }
    } catch (err) {
      if (err.response) {
  ...
  return (<>
     {loading && <Loading/>}
     {showModel && <Model setShowModel={setShowModel} msg={"Please login to add new Community"}/>}
